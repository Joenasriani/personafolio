...
export default function RunSafePage() {
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setStatus('Running safe crawl, extraction, and fallback generation...');

      const runResponse = await fetch(`/api/jobs-live/${created.jobId}/run-safe`, {
        method: 'POST',
      });
      const runResult = await runResponse.json();

      if (!runResponse.ok) {
        throw new Error(runResult.error || 'Safe pipeline run failed.');
      }

      setStatus(runResult.generated?.usedFallbackFacts ? 'Done using fallback signals. Opening results...' : 'Done. Opening results...');
  ...
  return (
    <main className="min-h-screen bg-background px-6 py-14 text-text">
      <div className="mx-auto max-w-3xl rounded-3xl border border-border bg-panel p-8 shadow-soft">
        <h1 className="text-3xl font-semibold">Run Personafolio safely</h1>
        <p className="mt-3 text-sm leading-7 text-muted">
          This runner uses fallback generation when structured facts are sparse or missing.
        </p>
        ...
        <form className="mt-8 space-y-5" onSubmit={handleSubmit}>
          ...
          <button type="submit" disabled={isSubmitting} className="rounded-2xl bg-accent px-5 py-3 font-medium text-background disabled:opacity-60">
            {isSubmitting ? 'Running…' : 'Run safe pipeline'}
          </button>
        </form>
