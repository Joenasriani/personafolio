...
export default function SubmitLivePage() {
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setStatus(response.ok ? `Live job created: ${result.jobId}` : result.error || 'Something went wrong');
    } catch (error) {
      setStatus('Request failed. Check environment variables and Supabase schema.');
    } finally {
  ...
  return (
    <main className="min-h-screen bg-background px-6 py-14 text-text">
      <div className="mx-auto max-w-3xl rounded-3xl border border-border bg-panel p-8 shadow-soft">
        <h1 className="text-3xl font-semibold">Create a live Personafolio job</h1>
        <p className="mt-3 text-sm leading-7 text-muted">
          This page writes directly to Supabase using the live jobs route.
        </p>
        ...
        <form className="mt-8 space-y-5" onSubmit={handleSubmit}>
          ...
          >
            {isSubmitting ? 'Submitting…' : 'Create live job'}
          </button>
        </form>
